...
def display_sector_comparison(data):
    ...
    try:
        sector_pe = float(benchmark["Average_PE"])
    except Exception:
        sector_pe = None

    try:
        sector_roe = float(benchmark["Average_ROE"])
    except Exception:
        sector_roe = None
    ...
    c1.metric("P/E القطاع", sector_pe if sector_pe else "غير متوفر")

    c2.metric("ROE الشركة", f"{round(company_roe * 100, 2)}%" if company_roe else "غير متوفر")
    c2.metric("ROE القطاع", f"{round(sector_roe * 100, 2)}%" if sector_roe else "غير متوفر")
